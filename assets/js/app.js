...
const CACHE = {
    ...
    set(key, data) {
        try { sessionStorage.setItem(this._prefix + key, JSON.stringify({ data, ts: Date.now() })); } catch { }
    },
    remove(key) {
        try { sessionStorage.removeItem(this._prefix + key); } catch { }
    },
    clear() {
        try {
...
const Security = {
    ...
    checkRateLimit(action) {
        ...
        let timestamps = [];
        try { const raw = localStorage.getItem(key); timestamps = raw ? JSON.parse(raw) : []; } catch { timestamps = []; }
        const now = Date.now();
...
const Auth = {
    ...
    async signUp(email, password, displayName) {
        try {
            ...
            try { await supabase.rpc('increment_user_count'); } catch { }
            try {
    ...
    async resetPassword(email) {
        try {
            ...
            return true;
        } catch { UI.toast('Something went wrong. Please try again.', 'error'); return false; }
    },
...
const DB = {
    ...
    reviews: {
        ...
        async submit({ groupId, rating, text }) {
            try {
                ...
                if (error) throw error;
                try { await supabase.rpc('update_review_stats', { p_group_id: groupId, p_new_rating: row.rating }); } catch { }
                try { await DB.user.addGXP(Auth.getUserId(), 20); } catch { }
    ...
    reports: {
        async submit({ groupId, reason, details }) {
            try {
                ...
                if (error) throw error;
                try { await DB.groups.incrementReports(groupId); } catch { }
                return data;
    ...
    ads: {
        ...
        async incrementImpressions(id) {
            try { await supabase.rpc('increment_ad_impressions', { p_ad_id: id }); } catch { }
        },
        async incrementClicks(id) {
            try { await supabase.rpc('increment_ad_clicks', { p_ad_id: id }); } catch { }
        }
    },
    stats: {
        async getGlobal() {
...
async function loadNotificationDropdown() {
    ...
    } catch { list.innerHTML = '<div style="padding:var(--space-4);text-align:center;color:var(--text-tertiary);font-size:var(--text-sm)">Unable to load</div>'; }
}
...
function openDrawer() {
    ...
    links += '<a href="/category" class="drawer__item">📂 Categories</a>';
    links += '<a href="/country" class="drawer__item">🌍 Countries</a>';
    links += '<a href="/platform" class="drawer__item">📱 Platforms</a>';
    links += '<div class="drawer__divider"></div>';
...
function renderFooter() {
    ...
    footer.innerHTML = '<div class="site-footer">' +
        ...
        '<a href="/category" class="site-footer__link">Categories</a>' +
        '<a href="/country" class="site-footer__link">Countries</a>' +
        '<a href="/platform" class="site-footer__link">Platforms</a>' +
...
function renderMobileNav() {
    ...
    nav.innerHTML = '<a href="/" class="mobile-nav__item' + (path === '/' ? ' active' : '') + '"><span class="mobile-nav__icon">🏠</span><span class="mobile-nav__label">Home</span></a>' +
        ...
        '<a href="/category" class="mobile-nav__item' + (path.startsWith('/category') || path.startsWith('/country') || path.startsWith('/platform') ? ' active' : '') + '"><span class="mobile-nav__icon">📂</span><span class="mobile-nav__label">Browse</span></a>' +
