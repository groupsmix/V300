...
const UI = {
    ...
    groupCard(group) {
        ...
        return '<div class="group-card" data-id="' + group.id + '">' +
            ...
            '<a href="/group?id=' + Security.obfuscateLink(group.id) + '" class="btn btn-primary btn-sm group-card__btn-visit">View Group</a>' +
    ...
    _initAuthModal(tab) {
        ...
        if (currentTab === 'signup' && CONFIG.turnstileSiteKey && typeof turnstile !== 'undefined') {
            var tw = document.getElementById('auth-turnstile-widget');
            if (tw) { turnstile.render('#auth-turnstile-widget', { sitekey: CONFIG.turnstileSiteKey, callback: function(token) { window._authTurnstileToken = token; } }); }
        }
